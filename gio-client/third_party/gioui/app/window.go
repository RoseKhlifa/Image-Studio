...
// FullscreenAuxiliary controls whether a window may accompany another window
// in a native fullscreen space.
//
// On macOS, enabled windows move to the active Space when shown and can appear
// with a fullscreen primary window. Other platforms currently ignore this
// option.
func FullscreenAuxiliary(enabled bool) Option {
	return func(_ unit.Metric, cnf *Config) {
		cnf.FullscreenAuxiliary = enabled
	}
}
