...
__attribute__ ((visibility ("hidden"))) CFTypeRef gio_createDisplayLink(CFTypeRef view);
...
func newDisplayLink(callback func(), view C.CFTypeRef) (*displayLink, error) {
	d := &displayLink{
		callback: callback,
	...
	dl := C.gio_createDisplayLink(view)
	if dl == 0 {
