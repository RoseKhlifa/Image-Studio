...
@interface GioDisplayLinkTarget : NSObject
@property(nonatomic, strong) CADisplayLink *displayLink;
@end

@implementation GioDisplayLinkTarget
- (void)onFrame:(CADisplayLink *)displayLink {
	gio_onFrameCallback((__bridge CFTypeRef)self);
}
@end

CFTypeRef gio_createDisplayLink(CFTypeRef viewRef) {
	if (@available(macOS 14.0, *)) {
		NSView *view = (__bridge NSView *)viewRef;
		if (view == nil) {
			return NULL;
		}
		GioDisplayLinkTarget *target = [[GioDisplayLinkTarget alloc] init];
		target.displayLink = [view displayLinkWithTarget:target selector:@selector(onFrame:)];
		if (target.displayLink == nil) {
			return NULL;
		}
		target.displayLink.paused = YES;
		[target.displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
		return (__bridge_retained CFTypeRef)target;
	}

	CVDisplayLinkRef dl = NULL;
	CVReturn result = CVDisplayLinkCreateWithActiveCGDisplays(&dl);
	if (result != kCVReturnSuccess || dl == NULL) {
		return NULL;
	}
	result = CVDisplayLinkSetOutputCallback(dl, displayLinkCallback, nil);
...
int gio_startDisplayLink(CFTypeRef dl) {
	if (@available(macOS 14.0, *)) {
		GioDisplayLinkTarget *target = (__bridge GioDisplayLinkTarget *)dl;
		dispatch_async(dispatch_get_main_queue(), ^{
			target.displayLink.paused = NO;
		});
		return kCVReturnSuccess;
	}
...
void gio_releaseDisplayLink(CFTypeRef dl) {
	if (@available(macOS 14.0, *)) {
		GioDisplayLinkTarget *target = CFBridgingRelease(dl);
		dispatch_async(dispatch_get_main_queue(), ^{
			[target.displayLink invalidate];
			target.displayLink = nil;
		});
		return;
	}
...
void gio_setDisplayLinkDisplay(CFTypeRef dl, uint64_t did) {
	if (@available(macOS 14.0, *)) {
		// NSView display links follow the view across displays automatically.
		return;
	}
