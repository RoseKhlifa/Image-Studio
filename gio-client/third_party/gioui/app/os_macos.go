...
static void setWindowCollectionBehavior(CFTypeRef windowRef, NSWindowCollectionBehavior controlledBehavior, int enabled) {
	@autoreleasepool {
		NSWindow *window = (__bridge NSWindow *)windowRef;
...
func (w *window) Configure(options []Option) {
	...
	window := C.windowForView(w.view)
	if w.config.FullscreenAuxiliary != cnf.FullscreenAuxiliary {
		w.config.FullscreenAuxiliary = cnf.FullscreenAuxiliary
		C.setWindowCollectionBehavior(
			window,
			fullscreenAuxiliaryCollectionBehavior(),
			C.int(boolToInt(cnf.FullscreenAuxiliary)),
		)
	}
...
func fullscreenAuxiliaryCollectionBehavior() C.NSWindowCollectionBehavior {
	return C.NSWindowCollectionBehaviorMoveToActiveSpace |
		C.NSWindowCollectionBehaviorFullScreenAuxiliary
}

func fullscreenAuxiliaryCollectionBehaviorBits() uint64 {
	return uint64(fullscreenAuxiliaryCollectionBehavior())
}
